type Question = {
  asked: boolean;
  question: string;
  answers: Answer[]; // oder answers: { text: string; correct: boolean }[];
};
...
let currentQuestion: Question | null | undefined = null;
let score = 0;
...
function createButtons(currentQuestion: Question) {
  if (!wrapper) return;
...
function handelButtonClick(antwort: Answer, button: HTMLButtonElement) {
  if (!antwort.correct) {
...
function loadFromLocalStorage() {
  if (storedQuestions) {
    storedQuestions.forEach((question: Question) => {
